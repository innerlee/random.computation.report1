...
\begin{table}[htb]
  ...
  \begin{tabular}{lllllllllll}
    \toprule
    algorithm & repeat & ref-$k$ & $k$ & prepare (s) & apply (s) & avg loss & min loss & max loss & median loss & std loss \\
    \midrule
...
\begin{equation}
    k=\Theta((d+\log(1/\delta))\epsilon^{-2}),
\end{equation}
$k$ is a multiple of $(21+log(10))\times100\approx2432$.
This is too large for real computation, so we set $k=100$.
...
\begin{equation}
    r=\Omega\big((\log(d))(\sqrt{d}+\sqrt{\log(n)})^2\epsilon^{-2}\big).
\end{equation}
A reference number for $k$ is
$\log(21)\times(\sqrt{21}+\sqrt{\log(2741220)})^2\times100\approx37234$.
This is too large for computation, thus we set $k=100$.
Results are shown in Table~\ref{tab:grand}.
